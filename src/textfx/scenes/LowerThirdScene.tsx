...
import {clamp01, easeOutCubic, lerp} from '../shared';
  ...
  return (
    <AbsoluteFill style={{background, fontFamily: 'Arial, "PingFang SC", sans-serif'}}>
      ...
      >
        ...
        <div>
          <div style={{fontSize: 88, fontWeight: 900, color: '#ffffff', lineHeight: 1.05}}>
            <TextFx
              key={active}
              effect={fx}
              text={entry.name}
              ...
              frame={frame}
              seed={active + 1}
              baseStyle={{fontSize: 88, fontWeight: 900, color: '#ffffff'}}
            />
          </div>
          <div style={{fontSize: 36, color: accent, marginTop: 10, letterSpacing: 2, opacity: roleOpacity}}>
            {entry.role}
          </div>
        </div>
      </div>
